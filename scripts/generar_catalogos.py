...
Busca columnas: Item, Pack, Size, Descripcion
"""
...
def generar_catalogos():
    ...
    # Genera un catalogo por cada fila
        ...
        # Agrega los datos disponibles en el Excel
              item = row.get('Item', 'N/A')
        pack = row.get('Pack', 'N/A')
        size = row.get('Size', 'N/A')
        descripcion = row.get('Descripcion', row.get('Descripción', 'N/A'))

        # Crea el contenido
        doc.add_paragraph(f"Item: {item}", style='Normal')
        doc.add_paragraph(f"Pack: {pack}", style='Normal')
        doc.add_paragraph(f"Size: {size}", style='Normal')
        doc.add_paragraph(f"Descripcion: {descripcion}", style='Normal')
